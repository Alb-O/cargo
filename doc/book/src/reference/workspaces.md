...
For a dynamically attached member, `cargo locate-project --workspace` reports the member manifest instead of the non-enumerating root manifest. Tools can pass that path back through `--manifest-path` to preserve the active workspace members. Metadata still reports the shared root in `workspace_root`.
