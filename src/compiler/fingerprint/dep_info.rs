...
pub fn translate_dep_info(
...
) -> CargoResult<(Vec<String>, Vec<PathBuf>)> {
    ...
    let observed_paths = on_disk_info
        .files
        .iter()
        .map(|(ty, path, _)| make_absolute_path(*ty, &pkg_root, &build_root, path.clone()))
        .collect();
    paths::write(cargo_dep_info, on_disk_info.serialize()?)?;
    Ok((observed_env, observed_paths))
}
